...
export default function EnvironmentVariablesTable() {
  ...
  const addEnvironmentVariable = () => {
    if (newEnvKey && newEnvValue) {
      setEnvironmentVariables([...environmentVariables, { key: newEnvKey, value: newEnvValue, type: envType }])
      setNewEnvKey("")
      setNewEnvValue("")
    }
  }
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center space-x-2 mb-4">
            <Switch
              ...
              onCheckedChange={(checked) => setEnvType(checked ? 'production' : 'preview')}
            />
            <Label htmlFor="envType">{envType === 'production' ? 'Production' : 'Preview'}</Label>
          </div>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {environmentVariables.filter(env => env.type === envType).map((env, index) => (
                <TableRow key={index}>
